...
const SaltLakeParikrama = () => {
  ...
  useEffect(() => {
    const fetchPandels = async () => {
      try {
        setLoading(true);
        const res = await axios.get("http://localhost:5000/api/pandel/zone/SaltLake & Central");
        setPandels(res.data);
      } catch (error) {
  ...
  return (
      ...
      <div className="px-4 py-3 bg-red-600 rounded-lg shadow-md mb-6 lg:mb-10">
        <h1 className="text-xl sm:text-2xl lg:text-3xl text-white font-serif font-semibold text-center lg:text-left">
          SaltLake Durga Puja Committees
        </h1>
      </div>
      ...
      {/* Grid Section */}
       {loading ? (
        ...
        <p className="text-center text-gray-600 dark:text-gray-400 py-10 italic">
          No pandels found in Salt Lake And Central Kolkata.
        </p>
      ) : (
...
export default SaltLakeParikrama