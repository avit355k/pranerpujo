...
const BonediBariParikrama = () => {
  const [pandels, setPandels] = useState([]);
  const [loading, setLoading] = useState(true);
  const [page, setPage] = useState(1);
  const itemsPerPage = 8;

  // ✅ Fetch pandels by type (Bonedi)
  useEffect(() => {
    const fetchPandels = async () => {
      try {
        setLoading(true);
        const res = await axios.get("http://localhost:5000/api/pandel/type/Bonedi Bari");
        setPandels(res.data);
      } catch (error) {
        if (error.response?.status === 404) {
          setPandels([]); // no pandels found
        } else {
          console.error("Error fetching Bonedi Bari pandels:", error);
        }
      } finally {
        setLoading(false);
      }
...
export default BonediBariParikrama;
